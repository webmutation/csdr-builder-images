...
ARG TEMURIN_BUILD=11.0.25_9
ARG JAVA_SDK_URL="https://github.com/adoptium/temurin11-binaries/releases/download/${TEMURIN_VERSION}/OpenJDK11U-jdk_x64_linux_hotspot_${TEMURIN_BUILD}.tar.gz"
...
ENV LANG=C.UTF-8 \
    ...
    NPM_VERSION=10.9.0 \
    JQ_VERSION=1.8.1 \
    PATH="/usr/lib/jvm/temurin-11/bin:/usr/share/maven/bin:$PATH"
...
RUN set -eux \
    && wget https://binaries.sonarsource.com/Distribution/sonar-scanner-cli/sonar-scanner-cli-${SONAR_SCANNER_VERSION}.zip -O /tmp/sonar-scanner.zip \
